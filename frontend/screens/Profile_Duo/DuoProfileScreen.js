...
import { getCurrentUser, getDuoMembers, getDuoMemberPhotos } from '../../utils/api';
    ...
    useCallback(() => {
      ...
      async function fetchAll() {
        ...
        try {
          ...
          // 2) if duo, fetch the members list
          if (me.profile_type === 'duo') {
            const { data: duoList } = await getDuoMembers(token);
            if (!isActive) return;
            setMembers(duoList);

            // 3) for each member, fetch their photos
            const newMap = {};
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.editBtn} onPress={() => navigation.navigate('EditDuoShared', { user })}>
      ...
      {members.map((m) => (
        <View key={m.id} style={styles.memberCard}>
          <View style={styles.infoContainer}>
            ...
            <Text>Ethnicity: {m.ethnicity}</Text>
            <Text>
              Personality:{' '}
          ...
          <TouchableOpacity onPress={() => navigation.navigate('EditDuoMember', { member: m })}>
